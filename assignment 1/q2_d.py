...
def read_input(name):
    ...
    with open(name) as f:
        reader = csv.reader(f)
        for row in reader:
            lst.append(np.asarray(row[1:]).astype(np.float))
    return lst
...
def calculate_output(curr_weights, curr_bias, curr_x):
    ...
    for i in range(len(curr_weights)):
        z = np.dot(np.asarray(output_list[i]).astype(np.float), np.asarray(curr_weights[i]).astype(np.float)) \
            + np.asarray(curr_bias[i]).astype(np.float)
    ...
    for i in range(len(dimensions)-2, 0, -1):
        ...
        for j in range(dimensions[i]):
            ...
            for k in range(dimensions[i+1]):
                g = g + gradients[i+1][k] * relu_gradient(z_list[i][j]) * np.float(current_weights[i][j][k])
