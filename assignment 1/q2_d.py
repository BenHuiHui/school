...
relative_path = "b"
weights_filenames = ["test.csv", "w-100-40-4.csv", "w-14*28-4.csv", "w-28*6-4.csv"]
bias_filenames = ["test-b.csv", "b-100-40-4.csv", "b-14*28-4.csv", "b-28*6-4.csv"]
output_bias_filenames = ["1.csv", "db-100-40-4.csv", "db-14*28-4.csv", "db-28*6-4.csv"]
output_weights_filenames = ["2.csv", "dw-100-40-4.csv", "dw-14*28-4.csv", "dw-28*6-4.csv"]
dims_list = [
    [2, 3],
    [14, 100, 40, 4],
...
def read_input(name):
    ...
    with open(name) as f:
        reader = csv.reader(f)
        for row in reader:
            lst.append(np.asarray(row[1:]).astype(np.float64))
    return lst
...
def read_weights(name, dimensions):
    ...
    for i in range(len(dimensions) - 1):
        ...
        if len(input_weights[current_row]) != dim_next:
            print ""
...
def calculate_output(curr_weights, curr_bias, curr_x):
    ...
    for i in range(len(curr_weights)):
        z = np.dot(np.asarray(output_list[i]).astype(np.float64), np.asarray(curr_weights[i]).astype(np.float64)) \
            + np.asarray(curr_bias[i]).astype(np.float64)
...
def calculate_softmax(output):
    output_e = np.exp(output - np.max(output))
    return output_e / output_e.sum()
    ...
    for i in range(len(dimensions)-2, 0, -1):
        ...
        for j in range(dimensions[i]):
            ...
            for k in range(dimensions[i+1]):
                g = g + gradients[i+1][k] * relu_gradient(z_list[i][j]) * np.float64(current_weights[i][j][k])
...
if __name__ == '__main__':
    for i in range(len(dims_list)):
        ...
        do_work(weights_filenames[i], bias_filenames[i],
                output_weights_filenames[i], output_bias_filenames[i],
                dims_list[i])
